...
// Simplified and robust query using posts(name: $slug)
...
// Reusable function to fetch post
...
export default async function PostDetail({ params }) {
  ...
  if (!post) {
    notFound(); // This will trigger 404 properly
  }
                    ...
                    <header className="email-header rounded-t-2xl w-full z-20 overflow-hidden">
                      <div className='nav-header rounded-t-2xl border border-solid border-theme-border w-full p-2 md:p-4 flex items-center justify-between flex-wrap gap-4 bg-white'>
                        ...
                        <div className="flex items-center relative">
                          ...
                          <div className='dark-light-mode block  bg-theme-border mx-2'></div>
                  ...
                  <div className='bg-white px-6 py-8 md:p-8 w-full md:rounded-2xl space-y-4'>
                    {post.postdata?.brand?.nodes?.[0] && (
                      <div className="flex items-center space-x-4">
                        ...
                        <h1>
                          <Link className="text-2xl font-bold block" href={`/${post.postdata.brand.nodes[0].slug}`}>
                            {post.postdata.brand.nodes[0].title}
                          </Link>
                        </h1>
                      </div>
                    )}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {post.emailTypes?.nodes?.map((tag, index) => (
                          <span key={index} className="text-base font-medium block leading-4 bg-theme-light-gray-2 text-theme-dark px-4 py-2 rounded-3xl">
                            {tag.name}
                          </span>
                        ))}
                        ...
                        {!hasAnyTaxonomy && !hasBrandCategories && (
                          <span className="text-base font-medium block leading-4 bg-theme-light-gray-2 text-theme-dark px-4 py-2 rounded-3xl">Other</span>
                        )}
                      </div>
                    </div>
                    <div>
                      <div className='flex items-center gap-3 border-t border-solid border-theme-border mt-6 pt-6 md:pt-8'>
                        <p className='text-base md:text-lg capitalize flex items-center gap-3 text-theme-dark'>
                          <Image src={DateIcon} alt="Date Icon" /> {formattedDate}
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <div className='py-10 md:pb-24 md:pt-1 related-category-post xl:px-4'>
          <div className="container">
            ...
            <div className="flex flex-wrap justify-center gap-2 md:gap-3">
              {post.emailTypes?.nodes?.map((tag, index) => (
                <Link
                  href={`/email-type/${tag.slug}`}
                  key={index}
                  className="text-sm md:text-base block leading-4 bg-white text-theme-dark hover:bg-theme-blue hover:text-white px-4 py-2 rounded-3xl font-medium"
                >
                  {tag.name}
                </Link>
              ))}
            ...
            <div className='text-center mt-12'>
              <Link href="/categories" className="inline-block bg-theme-blue text-white hover:bg-theme-dark font-semibold px-6 py-4 rounded-lg border-none text-base w-full md:w-auto">
                Explore All Email Templates
              </Link>
            </div>
          </div>
        </div>
